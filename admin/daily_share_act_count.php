          ...
          <div class="table-responsive">
            <div id="daily_topgirl_vote_count_div1" style="display:block">
              <table class="table table-hover">
                ...
                <tbody>
<?php
	$daily_date_query	= "SELECT sns_regdate FROM ".$_gl['share_info_table']." WHERE sns_user='act' Group by substr(sns_regdate,1,10) order by sns_regdate desc";
	$date_res			= mysqli_query($my_db, $daily_date_query);
	while($date_daily_data = mysqli_fetch_array($date_res))
	{
		$daily_date		= substr($date_daily_data['sns_regdate'],0,10);
		$media_query	= "SELECT sns_media, COUNT( sns_media ) media_cnt FROM ".$_gl['share_info_table']." WHERE sns_user='act' AND sns_regdate LIKE  '%".$daily_date."%' GROUP BY sns_media";
		...
		while ($media_daily_data = mysqli_fetch_array($media_res))
		{
			...
			$pc_query		= "SELECT * FROM ".$_gl['share_info_table']." WHERE sns_user='act' AND sns_regdate LIKE  '%".$daily_date."%' AND sns_media='".$media_daily_data['sns_media']."' AND sns_gubun='PC'";
			$pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
			$mobile_query	= "SELECT * FROM ".$_gl['share_info_table']." WHERE sns_user='act' AND sns_regdate LIKE  '%".$daily_date."%' AND sns_media='".$media_daily_data['sns_media']."' AND sns_gubun='MOBILE'";
