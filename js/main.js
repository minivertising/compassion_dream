...
function sns_share(media, flag, page)
{
	if (media == "fb")
	{
		...
		var newWindow = window.open('https://www.facebook.com/dialog/share?app_id=649187078561789&display=popup&href=' + encodeURIComponent('http://mydream.compassion.or.kr/PC/follower_index.php?used='+mb_rs+'_'+flag),'sharer','toolbar=0,status=0,width=600,height=325');
		$.ajax({
			type   : "POST",
			async  : false,
			url    : "../main_exec.php",
		...
		Kakao.Story.share({
			url: 'http://mydream.compassion.or.kr/PC/follower_index.php?used='+mb_rs+'_'+flag,
			text: '#내꿈꿔'
		});
		$.ajax({
			type   : "POST",
