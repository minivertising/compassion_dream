...
function sns_share(media, flag, page)
{
	if (media == "fb")
	{
		...
		var newWindow = window.open('https://www.facebook.com/dialog/share?app_id=649187078561789&display=popup&hashtag=내꿈꿔&href=' + encodeURIComponent('http://mydream.compassion.or.kr/MOBILE/follower_index.php?rs='+mb_rs+'&ugu=<?=$ugu?>'),'sharer','toolbar=0,status=0,width=600,height=325');
		$.ajax({
			type   : "POST",
			async  : false,
			url    : "../main_exec.php",
		...
		$.ajax({
			...
			success: function(response){
				...
				Kakao.Link.sendTalkLink({
				  ...
				  webButton: {
					text: '링크 열기',
					url: 'http://mydream.compassion.or.kr/MOBILE/follower_index.php?rs='+mb_rs+'&ugu=<?=$ugu?>' // 앱 설정의 웹 플랫폼에 등록한 도메인의 URL이어야 합니다.
				  }
				});
		...
		Kakao.Story.share({
			url: 'http://mydream.compassion.or.kr/MOBILE/follower_index.php?rs='+mb_rs+'&ugu=<?=$ugu?>',
			text: '#내꿈꿔'
		});
		$.ajax({
			type   : "POST",
