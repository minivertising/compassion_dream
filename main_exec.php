...
switch ($_REQUEST['exec'])
{
	...
	case "insert_share_info" :
		...
		$query 		= "INSERT INTO ".$_gl['share_info_table']."(sns_media, sns_ipaddr, sns_gubun, sns_user, inner_media, sns_regdate) values('".$sns_media."','".$_SERVER['REMOTE_ADDR']."','".$gubun."','".$mb_gubun."','".$_SESSION['ss_media']."','".date("Y-m-d H:i:s")."')";
	...
	case "insert_info" :
		...
		if ($dupli_arr)
		{
			...
			if ($check_choice === true)
			{
				...
				if ($result)
					$flag	= "Y||".$child_arr[1]."||".$mb_serial."||".$child_arr[2]."||".$child_arr[3]."||".$cName_post_position."||".$job_post_position."||".$child_arr[4]."||".$child_arr[5]."||".$child_arr[6]."||".$child_arr[7];
				else
					$flag	= "N||fail||N";
			...
			if ($result)
				$flag	= "Y||".$child_arr[1]."||".$mb_serial."||".$child_arr[2]."||".$child_arr[3]."||".$cName_post_position."||".$job_post_position."||".$child_arr[4]."||".$child_arr[5]."||".$child_arr[6];
			else
				//$flag	= "N||fail||N||N||N";
				$flag	= $query;
		}
